...
Predict coefficients and reconstruct shapes at specified positions in
model space.
...
- object:
  ...
  A stat\_\* object (stat_pca, stat_lda, stat_lm, ...)

- positions:

  A tibble with position coordinates. Column names must match the
  model's coordinate system (e.g., PC1, PC2 for PCA). Each row
  represents one position where coefficients will be reconstructed.
...
`transduce()` reconstructs coefficient vectors at specified positions in
model space and applies inverse transformations to obtain shapes.

The `positions` tibble defines where to reconstruct shapes. For PCA,
columns should be named PC1, PC2, etc. Unspecified PCs default to 0
(mean position).
